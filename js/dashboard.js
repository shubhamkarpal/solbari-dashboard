/**
 * Dashboard — renders pre-processed data from Apps Script
 */
...
const Dashboard = (() => {
  ...
  let trendChart = null;
  let revSplitChart = null;
  let brandGenChart = null;
  ...
  const fmtDelta = (cur, prev) => {
    ...
    return `<span class="${d >= 0 ? "delta-up" : "delta-down"}">${d >= 0 ? "▲" : "▼"} ${Math.abs(d).toFixed(1)}%</span>`;
  };
  const fmtDeltaInv = (cur, prev) => {
    if (cur == null || prev == null || prev === 0) return "";
    const d = ((cur - prev) / prev) * 100;
    return `<span class="${d <= 0 ? "delta-up" : "delta-down"}">${d >= 0 ? "▲" : "▼"} ${Math.abs(d).toFixed(1)}%</span>`;
  };
  ...
  async function init() {
    ...
    try {
      ...
      weeks.forEach((w, i) => {
        ...
        opt.value = i; opt.textContent = w; sel.appendChild(opt);
      });
      ...
      console.error(e);
      if (e.message.indexOf("Invalid response") >= 0) {
        showError("Could not connect to Apps Script. Make sure: (1) You deployed as a Web App, (2) Access is set to 'Anyone', (3) The URL in config.js is correct.");
      } else {
  ...
  async function loadWeek() {
    showLoading(`Loading ${weeks[currentWeekIdx]}...`);
    try {
      const data = await API.getWeekData(weeks[currentWeekIdx]);
      if (data.error) throw new Error(data.error);
      render(data.cur, data.prev);
      hideLoading();

      // Load trend in background
      API.getTrend().then(renderTrend).catch(() => {});
    } catch (e) {
      console.error(e);
      showError("Error loading week: " + e.message);
    }
  }
  ...
  function render(d, p) {
    ...
    document.getElementById("week-label").textContent =
      d.label + " Ad Performance" + (p ? ` (vs ${p.label})` : "");
  ...
  function setKPI(id, value, delta, sub) {
    ...
    el.querySelector(".kpi-delta").innerHTML = delta;
    const s = el.querySelector(".kpi-sub"); if (s) s.textContent = sub || "";
  }
  ...
  function renderTrend(data) {
    ...
    const labels = data.weekOrder || Object.keys(data.weeks);
    const salesData = labels.map((l) => data.weeks[l] ? data.weeks[l].sales : null);
    if (trendChart) trendChart.destroy();
    trendChart = new Chart(canvas, {
      type: "line",
      data: { labels, datasets: [{ label: "Ad Sales", data: salesData, borderColor: "#00d4ff", backgroundColor: "rgba(0,212,255,0.08)", fill: true, tension: 0.3, pointRadius: 5, pointBackgroundColor: "#00d4ff" }] },
      options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } },
        scales: { x: { ticks: { color: "#8b95a5" }, grid: { color: "rgba(255,255,255,0.05)" } },
          y: { ticks: { color: "#8b95a5", callback: (v) => "$" + (v >= 1000 ? (v / 1000).toFixed(0) + "K" : v) }, grid: { color: "rgba(255,255,255,0.05)" } } } },
    });
  }

  function renderRevSplit(adSales, organic) {
    const canvas = document.getElementById("rev-split-chart"); if (!canvas) return;
    if (revSplitChart) revSplitChart.destroy();
    revSplitChart = new Chart(canvas, { type: "doughnut",
      data: { labels: ["Ad Sales", "Organic"], datasets: [{ data: [adSales, organic], backgroundColor: ["#3b82f6", "#f97316"], borderWidth: 0 }] },
      options: { responsive: true, maintainAspectRatio: false, cutout: "65%", plugins: { legend: { position: "bottom", labels: { color: "#8b95a5", padding: 16, usePointStyle: true } } } },
    });
  }

  function renderBrandGen(brandSales, genericSales) {
    const canvas = document.getElementById("brand-gen-chart"); if (!canvas) return;
    if (brandGenChart) brandGenChart.destroy();
    brandGenChart = new Chart(canvas, { type: "doughnut",
      data: { labels: ["Brand", "Generic"], datasets: [{ data: [brandSales, genericSales], backgroundColor: ["#10b981", "#f97316"], borderWidth: 0 }] },
      options: { responsive: true, maintainAspectRatio: false, cutout: "65%", plugins: { legend: { position: "bottom", labels: { color: "#8b95a5", padding: 16, usePointStyle: true } } } },
    });
  }
  ...
  function renderSearchTerms(top, low, opp) {
    ...
    if (top && top.length) top.forEach((t) => {
      topBody.innerHTML += `<tr><td>${esc(t.term)}</td><td>${fmt(t.sales)}</td><td>${fmt(t.spend)}</td>
        <td class="${t.roas >= 5 ? 'text-green' : t.roas >= 3 ? 'text-yellow' : 'text-red'}">${t.roas}</td>
        <td>${t.orders}</td><td>${fmtN(t.clicks)}</td></tr>`;
    });
    const lowBody = document.getElementById("search-low-body"); lowBody.innerHTML = "";
    if (low && low.length) low.forEach((t) => {
      lowBody.innerHTML += `<tr><td>${esc(t.term)}</td><td class="text-red">${fmt(t.spend)}</td><td>${t.clicks}</td></tr>`;
    ...
    if (opp && opp.length) opp.forEach((t) => {
      oppBody.innerHTML += `<tr><td>${esc(t.term)}</td><td>${fmt(t.sales)}</td><td>${fmt(t.spend)}</td><td class="text-green">${t.roas}</td></tr>`;
    });
  }
  ...
  function renderProducts(products, prevProducts) {
    ...
    const prevMap = {};
    if (prevProducts) prevProducts.forEach((p) => { prevMap[p.asin] = p; });
    products.slice(0, 10).forEach((p) => {
      body.innerHTML += `<tr><td class="product-name">${esc(p.title)}</td><td>${fmt(p.sales)}</td><td>${fmt(p.spend)}</td>
        <td class="${p.roas >= 5 ? 'text-green' : p.roas >= 3 ? 'text-yellow' : 'text-red'}">${p.roas}</td><td>${p.orders}</td></tr>`;
    });
    if (prevProducts) {
      ...
      products.forEach((p) => {
        const prev = prevMap[p.asin]; if (!prev || prev.sales === 0) return;
        const wow = ((p.sales - prev.sales) / prev.sales * 100);
  ...
  function renderSummary(d, p) {
    const el = document.getElementById("exec-summary"); if (!el) return;
    const lines = [];
    if (d.sales && d.spend) {
    ...
    if (d.imp && p && p.imp) {
      const impD = ((d.imp - p.imp) / p.imp * 100).toFixed(0);
      const clkD = p.clicks ? ((d.clicks - p.clicks) / p.clicks * 100).toFixed(0) : 0;
