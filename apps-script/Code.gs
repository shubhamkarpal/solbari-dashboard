/**
 * Pattern Weekly Dashboard — API v6
 * Single combined CSV + Business Report per week.
 * Features: date ranges, auto-recommendations, funnel data
 */
...
function saveNotes(wk,summary,recs) { var p=PropertiesService.getScriptProperties();p.setProperty("s_"+wk,summary);p.setProperty("r_"+wk,recs);return{ok:true}; }

// ── Debug ────────────────────────────────────────────
function testDebug() { var w=getWeekList(); Logger.log(JSON.stringify(debugWeek(w[w.length-1]),null,2)); }
...
// ── Loaders ──────────────────────────────────────────
function loadWeekPair(weekName){var weeks=getWeekList(),idx=weeks.indexOf(weekName);var cur=gc("wk_"+weekName);if(!cur){cur=getWeekData(weekName);sc("wk_"+weekName,cur)}var prev=null;if(idx>0){var pw=weeks[idx-1];prev=gc("wk_"+pw);if(!prev){prev=getWeekData(pw);sc("wk_"+pw,prev)}}cur.mtd=getMTDData();var notes=getNotes(weekName);cur.savedSummary=notes.summary;cur.savedRecs=notes.recs;cur.autoRecs=generateRecommendations(cur,prev);return{brand:BRAND_NAME,weekOrder:weeks,cur:cur,prev:prev}}
...
function getWeekData(weekName){
  ...
  if(combined)r=mergeObj(r,parseCombinedCSV(combined,biz));
...
function parseCombinedCSV(txt,biz){
  var rows=Utilities.parseCsv(txt);if(rows.length<2)return{};
  ...
  if(dates.length>0){
    ...
    var first=formatDateShort(dates[0]),last=formatDateShort(dates[dates.length-1]);
  ...
  result.searchTermsTop=sorted.slice(0,15).map(function(e){var t=e[0],d=e[1];return{term:t,sales:R(d.sa),spend:R(d.s),roas:d.s>0?R(d.sa/d.s,1):0,orders:Math.round(d.o),clicks:Math.round(d.c)}});
...
function generateRecommendations(cur, prev) {
  ...
  // LOW PERFORMERS: search terms with wasted spend
  var lowCount = (cur.searchTermsLow || []).length;
  var lowSpend = 0;
  (cur.searchTermsLow || []).forEach(function(t) { lowSpend += t.spend; });
  if (lowCount > 0) {
    recs.push({ level: "HIGH", text: lowCount + " search terms with 30+ clicks haven't converted yet ($" + R(lowSpend) + " in spend). Adding as negative keywords to redirect that budget toward proven performers." });
  }

  // GENERIC SCALE: non-brand running at good ROAS
  var gc = cur.genericCampaigns || {};
  if (gc.roas && gc.roas >= 3) {
    recs.push({ level: "HIGH", text: "Non-brand campaigns running at " + gc.roas + "x ROAS — increasing daily budgets on top performers to capture additional impression share." });
  }

  // SCALING OPPS: high ROAS generic terms
  var topOpp = (cur.searchTermsOpp || [])[0];
  if (topOpp && topOpp.roas >= 10) {
    recs.push({ level: "MED", text: "\"" + topOpp.term + "\" converting at " + topOpp.roas + "x on $" + topOpp.spend + " spend — raising bids 15-20% to scale this opportunity." });
  }

  // HIGH-EFFICIENCY PRODUCTS
  var highEffProducts = (cur.products || []).filter(function(p) { return p.roas >= 8 && p.sales >= 500; });
  if (highEffProducts.length > 0) {
    var names = highEffProducts.slice(0, 2).map(function(p) { return p.title + " (" + p.roas + "x)"; }).join("; ");
    recs.push({ level: "MED", text: "Setting up dedicated campaigns for high-efficiency products: " + names + "." });
  }

  // ACOS ALERT
  if (cur.acos > 25) {
    recs.push({ level: "HIGH", text: "Overall ACOS at " + cur.acos + "% — reviewing bid strategy on underperforming campaigns to bring this closer to target." });
  }

  // NTB GROWTH
  if (cur.ntbOrders > 0 && cur.orders > 0) {
    var ntbPct = R(cur.ntbOrders / cur.orders * 100, 1);
    if (ntbPct > 50) {
      recs.push({ level: "LOW", text: "Strong acquisition: " + ntbPct + "% of orders are new-to-brand. Consider increasing generic keyword coverage to maintain this momentum." });
    }
  }

  // CVR CHECK
  if (cur.cvr && cur.cvr < 5 && prev && prev.cvr && cur.cvr < prev.cvr) {
    recs.push({ level: "MED", text: "CVR dropped to " + cur.cvr + "% from " + prev.cvr + "% last week. Reviewing listing content and pricing competitiveness." });
  }

  // COMPETITOR TARGETING
  recs.push({ level: "LOW", text: "Evaluating ASIN-level competitor targeting opportunities across top category products." });
